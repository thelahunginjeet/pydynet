...
class TestConstruction:
    ...
    def test_connect_full(self):
        ...
        assert len(net.edges()) == self.N*(self.N-1)/2, 'FULLY CONNECTED: Wrong number of edges!'
        assert np.allclose(self.N-1,net.degree().values()), 'FULLY CONNECTED: Some nodes have the wrong degree!'
    ...
    def test_connect_ring(self):
        ...
        assert np.allclose(2,net.degree().values()), 'RING: Some nodes have the wrong degree!'
    ...
    def test_connect_fixed_degree(self):
        ...
        assert len(net.edges()) == (int(p*self.N)*self.N)/2, 'FIXED DEGREE: Wrong number of edges!'
        assert np.allclose(int(p*self.N),net.degree().values()), 'FIXED DEGREE: Some nodes have the wrong degree!'
    ...
    def test_connect_fixed_edges(self):
        ...
        assert len(net.edges()) == int(p*self.N*(self.N-1)/2), 'FIXED EDGES: Wrong number of edges!'
